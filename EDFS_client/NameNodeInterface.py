...
class NameNodeInterface:
    ...
    def get_file_partitions(self, file_path):
        response = requests.post(self.nameNode_url, json={"command": "get_file_partitions",
                                                          "content": {"filepath": file_path}})
        return response.json()

    def ls(self, file_path):
        response = requests.post(self.nameNode_url, json={"command": "ls",
                                                          "content": {"filepath": file_path}})
        return response.json()

    def get_partition_table_name(self, file_path, partition_no):
        response = requests.post(self.nameNode_url, json={"command": "get_partition_table_name",
                                                          "content": {"file_path": file_path,
